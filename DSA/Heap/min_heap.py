...
class MinHeap:
    ...
    def _heapify_down(self, idx: int = None):
        ...
        if right < len(self._heap):
            smallest_idx = left if self._heap[left] < self._heap[right] else right

        if smallest_idx and self._heap[smallest_idx] < self._heap[idx]:
            self._heap[idx], self._heap[smallest_idx] = self._heap[smallest_idx], self._heap[idx]
            self._heapify_down(smallest_idx)
